...
export default {
  name: "Student",
  // props: ['name', 'sex', 'age'], //简单声明接收，开发中用的较多

  // props: ['name', 'sex', 'age', "phone"], //没有传的属性，值为undefined
  ...
  //接收时不仅限制类型，还加上默认值的指定，并指定它的必须性
  props:{
    name:{
      type: String, //类型
      required: true //必要的,一般required和default不会同时出现
    },
    age:{
      type: Number,
      default: 99 //默认值
  ...
  methods:{
    updateName(){
      //this.name = "222";//能修改，会生效，但是会报错，不要直接修改外部传过来的数据
      this.myName = 'sss';
    }
  },
}
